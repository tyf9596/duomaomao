...
- `Assets/Game/Scenes/Arena06.unity` — **second map "The Mansion"** (48×42m, in build
  ...
  **stairwell down**. 2ND FLOOR (west wing + east wing, floors 3.2..3.49, connected via
  ...
  (desks split, chairs pulled out). **Mega-fill 2026-07-21** (user: still too empty; via
  `Assets/Editor/MansionFillPass.cs` menu passes since execute_code is dead): floater
  settle pass (19 props dropped onto their supports) + overlap nudger; ~120 more props —
  ballroom banquet row + 8 theater chairs + rug, library wall-bookcases + reading nook +
  globe, kitchen upper cabinets + 2nd counter run + cookware, pantry sacks/shelves,
  bedroom dresser/lamps/pillows, bathroom towels + mirror, storage 2-tier shelving units
  + pallets, balcony seats, study cubicle grid + filing + papers, kids play table + more
  toys, attic junk mountain (stacked chairs, rolled rugs, fallen coat rack), lounge bar +
  dartboard, basement pipes/coal/clothesline bottles, courtyard planters + picnic +
  birdbath + mural benches; **protruding wall art** (deep-relief framed paintings w/
  ledges, 2 stone trophy busts flanking the stage, wall shelves w/ bottles, hallway
  clock, bathroom mirror); **graffiti objectified**: 6 leaning graffiti boards, 4 floor
  splat discs, 2 graffiti-bombed crates, a corner-wrap piece. `Tools/Mansion/V0` turns
  on `Application.runInBackground` — play mode keeps running when ANOTHER Unity editor
  steals OS focus (two agents testing at once). Pattern floors are per-room .mat tiling VARIANTS (cube UVs are 0..1 per
...
- `Assets/Game/Resources/CharacterAnimator.controller` — shared controller built by editor
  ...
  identical across variants). Character rig is 2.7m tall → **asymmetric sizes since
  2026-07-21 (user call): hiders `Character.HiderScale 0.4` = 1.08m, hunter
  `HunterScale 0.5` = 1.35m** — everyone spawns hider-sized, `Character.ApplySize`
  resizes body + CC + EyePos, `MakeHunter`/conversion GROWS the infected. Scenery
  `DecoyStatue.bodyScale` default 0.4 matches hiders (the Arena05 monument markers
  were rescaled ×1.25 to compensate). CC scales with size; cam pivot 1.1.
...
- Poses (9): Stand / Crouch(sit clip) / Statue(static) / Lie(static + body tipped -90°,
  ...
  **Ball**(sit clip + procedural hands-on-head bone offsets in `CharacterMotor.LateUpdate`
  ...
  Pose int drives animator. **Poses PERSIST through movement since 2026-07-21 (user
  call)** — posed characters waddle/glide around like the original game; picking
  NORMAL (pose 0, first picker tile) restores locomotion anims. Bot hunters force
  Stand in `HuntBehavior` so nobody stalks in a lobby pose. Touch UI: POSE button
  opens a 2-column 9-option picker (PlayerRig `_posePanel`); editor P key cycles.
...
## UI redesign — IMPLEMENTED (2026-07-21, INK & PAINT 1a + resultB)

The Claude Design redesign (claude.ai/design project e327d732…, canvas
`MecchaChameleon-UI-Redesign.dc.html`, direction **1a INK & PAINT** with the **1b
resultB** cream-poster result screen) is fully implemented and play-verified via
UiShotDirector (all 11 shots match the canvas):

- **UiKit v2** owns the design system: token colors (`UiKit.Ink/Blue/HunterRed/Gold/…`),
  Baloo 2 fonts (`BoldFont` 800 = titles/buttons, `DefaultFont` 600 = body;
  `Resources/Fonts/`, SIL OFL), sprite/icon loaders (`Shape()`/`Icon()`),
  `MakeStickerButton` (drop shadow + fill + dark bottom crescent + silhouette icon +
  label), `MakePill/MakeCard/MakePanel/MakeRainbowStrip`, and `PressFx`
  (press scale .92 + sink + darken, release overshoot).
- **Assets** (all white, tinted at runtime via Image.color): 26 icons + 9-slice
  panels/pills/tiles/rings/stripe-tile/vignette in `Assets/Game/Resources/UI/`
  (import settings enforced by `Assets/Editor/UiArtImporter.cs`). Icon silhouettes were
  rasterized from the SVG paths embedded in the design canvas (svglib, white-on-black →
  luminance-as-alpha); regenerate with a similar script if icons change.
- Per-screen: banner card w/ rainbow underline (hunter mood = red + warning stripes),
  timer pill (SEEK/last-10s = red + heartbeat), STYLE badge w/ gold ring + "+N" float,
  CREAM roster card (dot avatars, YOU row, HUNT badges), full-width reveal/warn plates,
  JUMP charge ring + HOLD=CLIMB, 3×3 pose panel w/ pose silhouettes + scene dim
  (bottom clears PAINT by 45px), real FPS crosshair w/ 1.1s cooldown ring + red
  vignette, paint toolbar (hand-positioned — runtime nested LayoutGroups proved
  unreliable, see SelfPaintMode.BarRect), resultB poster (opaque cream + confetti +
  ink-bordered cards + medals; controls hidden during RESULT).
- **Gotchas**: images inside a LayoutGroup need `LayoutElement.ignoreLayout=true` for
  stretch backdrops; Baloo 2 needs `Text.lineSpacing ≈ 0.78` on multi-line titles;
  keep loading-screen splats out of the text/TIP bands (`SpawnSplat` keep-out).
- **Motion pass (same day, spec section 5 complete)**: `UiKit.PopIn/SlideIn/Fade` +
  `EaseOutBack/EaseOutCubic` micro-tween helpers drive every transition — banner card
  in/out (SetTitle), roulette freeze white-flash + plate shake, reveal fade-out, warn
  plate pop, last-3s ink vignette (SetTimer), roster row slide-in on join, FlashBanner
  ink pill backdrop, resultB entrance sequence (mask → title pop → score pop → rows
  staggered → PLAY AGAIN; confetti falls only on a hiders win), PLAY AGAIN ink-fade
  scene reload, pose panel pop + dim fade (auto-closes on team/phase change and when
  paint mode opens), WAIT context state is non-interactable, crosshair recoil kick on
  fire, spectate camera cuts hide behind a 0.1s ink blink, loading roller bobs. With
  TWO editors open, `set_active_instance MecchaChameleon@07b21147` first; the focus
  scripts of concurrent agents fight over the foreground — expect stalls, the shot
  director's game-time stall guard just waits them out.

- `Docs/UI/UI-Redesign-Requirements.md` — the ORIGINAL requirements doc (pre-redesign
  UI inventory; its `shots/` links now show the POST-redesign captures — the .docx
  keeps the pre-redesign images embedded as the before-archive) + `Docs/UI/shots/` =
  11 live 1080×1920 screenshots regenerated by `Assets/Editor/UiShotDirector.cs`
  (menu `Tools/UI Shots/Run Full Session`, or drop `Docs/UI/shots/_arm.txt` containing
  `autoplay` and let the next domain reload run it).
...
- **The editor barely pumps play-mode frames without OS focus** (frameCount freezes; MCP
  ...
  checked into the repo so it survives sessions; since 2026-07-21 it targets THIS
  project's editor when several Unity windows are open — `-Project <name>` overrides,
  default = repo folder name, title match with cmdline fallback while compiling),
